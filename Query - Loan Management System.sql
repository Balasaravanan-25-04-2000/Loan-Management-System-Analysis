-- STEP 1
-- Create table customer criteria based on applicant income.
...
ORDER BY ApplicantIncome;

SELECT * FROM Customer_criteria;

-- STEP 2
-- Calculate monthly interest amount and annual interest amount based on loan amount.
...
ELSE "7%"
END AS Monthly_interest_percentage_criteria
...
ADD Monthly_interest_amount FLOAT,
ADD Annual_interest_amount FLOAT;

DROP TABLE Customer_interest_analysis;

DESCRIBE Customer_interest_analysis;

UPDATE Customer_interest_analysis
SET Monthly_interest_amount = CASE
WHEN ApplicantIncome < 5000 AND Property_Area = "Rural" THEN ((ApplicantIncome * 3) / 100)
WHEN ApplicantIncome < 5000 AND Property_Area = "Semirural" THEN ((ApplicantIncome * 3.5) / 100)
WHEN ApplicantIncome < 5000 AND Property_Area = "Urban" THEN ((ApplicantIncome * 5) / 100)
WHEN ApplicantIncome < 5000 AND Property_Area = "Semiurban" THEN ((ApplicantIncome * 2.5) / 100)
ELSE ((ApplicantIncome * 2.5) / 100)
END;

UPDATE Customer_interest_analysis
SET Annual_interest_amount = CASE
WHEN ApplicantIncome < 5000 AND Property_Area = "Rural" THEN (((ApplicantIncome * 3) / 100) * 12)
WHEN ApplicantIncome < 5000 AND Property_Area = "Semirural" THEN (((ApplicantIncome * 3.5) / 100) * 12)
WHEN ApplicantIncome < 5000 AND Property_Area = "Urban" THEN (((ApplicantIncome * 5) / 100) * 12)
WHEN ApplicantIncome < 5000 AND Property_Area = "Semiurban" THEN (((ApplicantIncome * 2.5) / 100) * 12)
ELSE (((ApplicantIncome * 2.5) / 100) * 12)
END;

-- STEP 3
-- Create row level trigger for loan amount (Loan amt null = loan still processing).
...
BEFORE INSERT ON Customer_interest_analysis
FOR EACH ROW
BEGIN
...
ALTER TABLE Customer_interest_analysis
ADD Cibil_Customer_type VARCHAR(50);

ALTER TABLE Customer_interest_analysis
ADD `Cibil Score` INT;

ALTER TABLE Customer_interest_analysis
DROP Cibil_Customer_type;

ALTER TABLE Customer_interest_analysis
DROP `Cibil Score`;

DELIMITER //
CREATE TRIGGER Cibil_Customer_type
AFTER INSERT ON loan_status
FOR EACH ROW
BEGIN
IF NEW.`Cibil Score` > 900 THEN UPDATE Customer_interest_analysis SET Cibil_Customer_type = "High cibil score"
WHERE Loan_ID = NEW.Loan_ID;
ELSEIF NEW.`Cibil Score` > 750 THEN UPDATE Customer_interest_analysis SET Cibil_Customer_type = "No penalty"
WHERE Loan_ID = NEW.Loan_ID;
ELSEIF NEW.`Cibil Score` > 0 THEN UPDATE Customer_interest_analysis SET Cibil_Customer_type = "Penalty customers"
WHERE Loan_ID = NEW.Loan_ID;
ELSEIF NEW.`Cibil Score` <= 0 THEN UPDATE Customer_interest_analysis SET Cibil_Customer_type = "Reject customers"
WHERE Loan_ID = NEW.Loan_ID;
END IF;
END //
DELIMITER ;

DROP TRIGGER Cibil_Customer_type;

SHOW TRIGGERS;

INSERT INTO Customer_interest_analysis
SELECT I.*,
CASE
WHEN ApplicantIncome > 15000 THEN "Grade A customer"
WHEN ApplicantIncome > 9000 THEN "Grade B customer"
...
END AS Monthly_interest_percentage_criteria,
CASE
WHEN ApplicantIncome < 5000 AND Property_Area = "Rural" THEN ROUND(((ApplicantIncome * 3) / 100),2)
WHEN ApplicantIncome < 5000 AND Property_Area = "Semirural" THEN ROUND(((ApplicantIncome * 3.5) / 100),2)
WHEN ApplicantIncome < 5000 AND Property_Area = "Urban" THEN ROUND(((ApplicantIncome * 5) / 100),2)
WHEN ApplicantIncome < 5000 AND Property_Area = "Semiurban" THEN ROUND(((ApplicantIncome * 2.5) / 100),2)
ELSE ROUND(((ApplicantIncome * 2.5) / 100),2)
END AS Monthly_interest_amount,
CASE
WHEN ApplicantIncome < 5000 AND Property_Area = "Rural" THEN ROUND((((ApplicantIncome * 3) / 100) * 12),2)
WHEN ApplicantIncome < 5000 AND Property_Area = "Semirural" THEN ROUND((((ApplicantIncome * 3.5) / 100) * 12),2)
WHEN ApplicantIncome < 5000 AND Property_Area = "Urban" THEN ROUND((((ApplicantIncome * 5) / 100) * 12),2)
WHEN ApplicantIncome < 5000 AND Property_Area = "Semiurban" THEN ROUND((((ApplicantIncome * 2.5) / 100) * 12),2)
ELSE ROUND((((ApplicantIncome * 2.5) / 100) * 12),2)
END AS Annual_interest_amount,
L.LoanAmount, L.Loan_Amount_Term, L.`Cibil Score` FROM customer_income I
INNER JOIN loan_status L
...
TRUNCATE Customer_interest_analysis;
...
DELETE FROM customer_interest_analysis
WHERE LoanAmount = "Loan still processing" OR
Cibil_Customer_type = "Reject customers";

ALTER TABLE customer_interest_analysis
MODIFY LoanAmount INT;

CREATE TABLE Loan_cibil_score_status_details
SELECT I.*, D.Gender, D.Age
FROM customer_interest_analysis I
INNER JOIN customer_det D
ON I.Loan_ID = D.Loan_ID;
...
CREATE TABLE OPT_1_1
SELECT L.*, D.Customer_name, D.Married, D.Education, D.Self_Employed,D.Region_id
FROM Loan_cibil_score_status_details L
INNER JOIN customer_det D
ON L.Loan_Id = D.Loan_Id;

SELECT * FROM OPT_1_1;

CREATE TABLE OPT_1_2
SELECT O.*, S.Postal_Code, S.Segment, S.State
FROM OPT_1_1 O
INNER JOIN country_state S
...
SELECT * FROM OUTPUT_1
WHERE Cibil_Customer_type = "High cibil score";
